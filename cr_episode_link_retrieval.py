...
#since the code is meant to complement youtube-dl and it's archiving feature this is only
#used for getting links and then passing them to youtube-dl to download
#so thats why instead of my original idea of using it as an archive I've opted to 
#rely on youtube-dl for that
open(crel, 'w')
episode_list = open(crel, 'r')
current_episode_list = episode_list.readlines()
# c = 0
# for x in current_episode_list:
#     if 'https://www.crunchyroll.com/a-certain-magical-index/episode-16-the-governing-board-781242' in x:
...
with webdriver.Firefox(options=fireFoxOptions) as driver:
    ...
    for series in cvl:
        ...
        # print(6)
        for verification in starts:
            # print(7)
            verify = verification.get_attribute('href')
            # print(8)
            # print(verify)
            for current in range(0, len(current_episode_list)):
                # print(9)
                # print(current_episode_list)
                if verify + '\n' in current_episode_list[current]:
                    # print(10)
                    print('[verify', current, ']', verify + '\n')
                    # print(11)
                    break
                else:
                    print('writing', verify)
                    episode_list = open(crel, 'a')
                    episode_list.write(verify + '\n')
                    break
        continue
...
quit()
    ...
    #     for verify in current_episode_list:
    #         if end in verify:
    #             print('[archived]', end)
    #             break       
    ...
    #         if end.startswith(cvl[count] + '/episode'):
    #             episode_list = open(crel, 'a')
